  ...
  if (stats) {
    if (preserveOption === 'append') {
      try {
        ...
        console.log(`  ${colors.red}Error appending contents of ${path.basename(targetFile)}: ${e.message}${colors.reset}`);
      }
      backupFile(targetFile);
    } else if (preserveOption === 'backup') {
      const backupPath = backupFile(targetFile);
      if (backupPath && backupPath !== 'symlink_removed') {
        console.log(`  ${colors.dim}Backed up existing file to ${path.basename(backupPath)}${colors.reset}`);
      }
    } else if (preserveOption === 'overwrite') {
      try {
        fs.unlinkSync(targetFile);
      } catch (e) {
        try { fs.rmSync(targetFile, { force: true }); } catch (err) {}
      }
    } else {
      // Skip
...
function isLinked(sourceFile, targetFile) {
  try {
    ...
    if (lstats.isSymbolicLink()) {
      ...
      const resolvedLinkTarget = path.resolve(path.dirname(targetFile), linkTarget);
...
function removeAiderConfig() {
  ...
  const filteredLines = lines.filter(line => !line.includes('.aider.conventions.md'));
...
└────────────────────────────────────────────────────────┘
  ...
  console.log(`Global instructions path: ${colors.bold}${sourceFile}${colors.reset}\n`);
  ...
  if (!fs.existsSync(sourceFile)) {
    const confirmPrompt = await prompts({
      type: 'confirm',
      name: 'create',
      message: 'Global instructions file does not exist. Create it with a standard template?',
      initial: true
    }, { onCancel });
    ...
    if (confirmPrompt.create) {
      ...
      console.log(`${colors.green}Created global instructions file at ${sourceFile}${colors.reset}\n`);
    } else {
      console.log(`${colors.red}Abort: Global instructions file is required to proceed.${colors.reset}`);
      process.exit(1);
    }
  }
  ...
  const choices = tools.map((t) => {
    ...
    return {
      title: t.name,
      value: t,
      description: `${t.path} ${linked ? colors.cyan + '(linked)' + colors.reset : ''}`.trim(),
      selected: linked
    };
  });
  ...
  const previouslyLinked = tools.filter(t => isLinked(sourceFile, resolvePath(t.path)));

  // Tools to link: selected ones that are NOT currently linked
  const newToolsToLink = selectedTools.filter(t => !isLinked(sourceFile, resolvePath(t.path)));

  // Tools to unlink: previously linked ones that are NOT selected anymore
  const toolsToUnlink = previouslyLinked.filter(t => !selectedTools.some(sel => sel.id === t.id));
  ...
  if (toolsToUnlink.length > 0) {
    ...
    for (const tool of toolsToUnlink) {
      ...
      console.log(`  Unlinking ${colors.bold}${tool.name}${colors.reset}...`);
      try {
        ...
        console.log(`    ${colors.green}Unlinked successfully.${colors.reset}`);

        if (tool.id === 'aider') {
          try {
            removeAiderConfig();
            console.log(`    ${colors.dim}Cleaned Aider conventions from ~/.aider.conf.yml.${colors.reset}`);
          } catch (e) {
            console.log(`    ${colors.yellow}Could not clean ~/.aider.conf.yml: ${e.message}${colors.reset}`);
          }
        }
      } catch (e) {
        console.log(`    ${colors.red}Failed to unlink: ${e.message}${colors.reset}`);
      }
    }
  }
  ...
  if (newToolsToLink.length > 0) {
    ...
    const preservePrompt = await prompts({
      type: 'select',
      name: 'option',
      message: 'If a target instructions file already exists for new agents, what should we do?',
      choices: [
        { title: 'Append & Link', value: 'append', description: 'Append existing instructions to Source of Truth, back up the file, and link' },
        { title: 'Backup & Link', value: 'backup', description: 'Rename existing file to .bak and link' },
        { title: 'Overwrite & Link', value: 'overwrite', description: 'Directly delete existing file and link' },
        { title: 'Skip', value: 'skip', description: 'Do not modify the file for this agent' }
      ]
    }, { onCancel });
    ...
    for (const tool of newToolsToLink) {
      ...
      if (result.skipped) {
        ...
        let typeLabel = '';
        if (result.type === 'symlink') typeLabel = `${colors.green}Symlink created successfully.${colors.reset}`;
        if (result.type === 'hardlink') typeLabel = `${colors.cyan}Hard link created successfully (Windows symlink fallback).${colors.reset}`;
        if (result.type === 'copy') typeLabel = `${colors.yellow}Copy created successfully (linking not possible on this environment).${colors.reset}`;
        console.log(`  ${typeLabel}`);
      } else {
        console.log(`  ${colors.red}Failed: ${result.error ? result.error.message : 'Unknown error'}${colors.reset}`);
      }
    ...
    if (aiderSelected) {
      ...
      const configureAiderPrompt = await prompts({
        type: 'confirm',
        name: 'configure',
        message: 'Would you like to auto-update ~/.aider.conf.yml to load ~/.aider.conventions.md?',
        initial: true
      }, { onCancel });
      ...
      if (configureAiderPrompt.configure) {
        try {
          ...
          if (aiderRes === 'updated') {
            console.log(`  ${colors.green}Successfully updated ~/.aider.conf.yml.${colors.reset}`);
          } else if (aiderRes === 'already_configured') {
            console.log(`  ${colors.dim}Already configured in ~/.aider.conf.yml.${colors.reset}`);
          }
        } catch (e) {
          console.log(`  ${colors.red}Failed to update ~/.aider.conf.yml: ${e.message}${colors.reset}`);
        }
      }
    }
  }
...
main().catch(err => {
  console.error(`${colors.red}An error occurred: ${err.message}${colors.reset}`);
  process.exit(1);
});
